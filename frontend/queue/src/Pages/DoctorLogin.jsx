...
const DoctorLogin = () => {
    ...
    e.preventDefault();
    try {
      const response1 = await axios.post(`${BackendUrl}/doctor/login`, {
        "input":phoneNo,
        "password":password,
      });
    ...
    const handleRegister = async(e) =>{
        e.preventDefault();
        try{
            const registerResponse=await axios.post(`${BackendUrl}/doctor/register`,{
                "name":name,
    ...
    const handleSendOtp = async(e)=>{
        e.preventDefault();
        try{
            const sendOtpResponse=await axios.post(`${BackendUrl}/doctor/sendOtp/${phoneNo}`);
            if(sendOtpResponse){
                setotpSend(1);
            }
        }
    ...
    const handleLoginViaOtp=async(e)=>{
        e.preventDefault();
        try{
            const res=await axios.post(`${BackendUrl}/doctor/loginViaOtp`,
                {
                    "input":phoneNo,
                    "password":enteredOtp
                }
