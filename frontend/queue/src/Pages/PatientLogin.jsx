...
const PatientLogin = () => {
    ...
    async function handleLogin() {
        try{
            const response=await axios.post(`${BackendUrl}/patient/login`,
                {
                    "input":phoneNo,
                    "password":password
                }
    ...
    async function handleRegister() {
        try{
            const registerResponse=await axios.post(`${BackendUrl}/patient/register`,
                {
                    "name":name,
                    "phoneNumber":phoneNo,
                    "email":email,
    ...
    async function handleSendOtp() {
        try{
            const isSent=await axios.post(`${BackendUrl}/patient/sendOtp/${phoneNo}`);
            if(isSent){
                setotpSend(1);
            }
        }
    ...
    async function submitOtp() {
        try{
            const PatientDTO=await axios.post(`${BackendUrl}/patient/loginViaOtp`,
                {
                    "input":phoneNo,
                    "password":password
                }
